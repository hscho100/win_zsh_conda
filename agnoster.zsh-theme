...
prompt_context() {
  if [[ "$USERNAME" != "hscho" || -n "$SSH_CLIENT" ]]; then
...
prompt_virtualenv() {
  ...
  if [[ -n $VIRTUAL_ENV ]]; then
    local env_label=" $(basename ${VIRTUAL_ENV:t:gs/%/%%}) "
  fi
  ...
  if [[ -n $env_label ]]; then
    ...
    fg_color=black
    CONDA_ICON_PREFIX=🐍
    prompt_segment $color $fg_color $CONDA_ICON_PREFIX 
    print -Pn $env_label
  fi
}
# prompt_virtualenv() {
...
prompt_cmd_exec_time() {
  [ $BULLETTRAIN_last_exec_duration -gt $BULLETTRAIN_EXEC_TIME_ELAPSED ] && prompt_segment $BULLETTRAIN_EXEC_TIME_BG $CURRENT_FG "$(displaytime $BULLETTRAIN_last_exec_duration)"
}

prompt_time() {
  if [[ $BULLETTRAIN_TIME_12HR == false ]]; then
